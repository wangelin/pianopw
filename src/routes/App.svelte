<script>
	...
	let n_low_white = $derived(n_low - (number_is_flat_or_sharp(n_low) ? 1 : 0));
	let n_high = $state(note_to_number("C6"));
	let high_key = $derived(number_to_letter(n_high)); // 88
	let n_high_white = $derived(n_high + (number_is_flat_or_sharp(n_high) ? 1 : 0));

	let number_of_white_keys = $derived(white_keys_up_to(n_high_white) - white_keys_up_to(n_low_white) + 1)
	let white_keys_per_row = $derived(Math.ceil(number_of_white_keys / rows))
	
	let canvas = $state();

	let loop_click = $state();
	...
	function get_note (rect, x, y) {
		...
		const clicked_note_index = n_low_white - note_offset + octaves * 12 + white_key_index_to_note_index((clicked_pos + white_key_offset) % 7);
		...
		let hit = point_inside_polygon([internal_x, internal_y], white_key_shapes[shape_name])
		let hit_note = "";
		status = clicked_note
		if (hit) {
			// clean hit
			...
			if (note_name.match(/^[DEGAB]$/)) {
				hit = point_inside_polygon([internal_x, internal_y], black_key_shapes[note_name])
				if (hit) {
			...
			if (!hit && note_name.match(/^[CDFGA]$/)) {
				...
				hit = point_inside_polygon([internal_x, internal_y], black_key_shapes[next_note].map(([x, y]) => [x + 1, y]))
				if (hit) {
					return number_to_note(clicked_note_index + 1);
				}
			}
	...
	onMount(() => {
		...
		function loop() {
			...
			for (let r = 0; r < rows; r++) {
				let x = 0;
				for (let i = 0, w = 0; w < white_keys_per_row && k <= n_high_white; i++) {
					if (number_is_natural(k)) {
						w++;
						let letter = number_to_letter(k);
						let note = number_to_note(k);
						if (r > 0 && i === 0 && number_is_flat_or_sharp(k - 1)) {
							let letter = number_to_letter(k - 1);
							let note = number_to_note(k - 1);
							draw_black_key(ctx, letter, x, y, key_width, canvas.height / rows, { text: show_text ? note : "" });
							if (note === playing_note) {
								draw_black_key(ctx, letter, x, y, key_width, canvas.height / rows, { fillStyle: "rgba(255,255,255,0.2)" });
							}
						}
						if (k === n_low_white) letter += "0"
						else if (k === n_high_white) letter += "1";
						draw_white_key(
							ctx, letter, x, y, key_width, canvas.height / rows,
							{
								text: show_text ? note : "",
						...
						if (mark_middle_c && note === "C4") {
							draw_white_key(ctx, letter, x, y, key_width, canvas.height / rows, { fillStyle: "rgba(0, 0, 0, 20%)" });
						}
						if (note === playing_note) {
							draw_white_key(ctx, letter, x, y, key_width, canvas.height / rows, { fillStyle: "rgba(0, 0, 0, 10%)" });
						}
						x += key_width;
						k++;
...
<svelte:window onpointerup={playing_note ? onpointerup : undefined} onpointermove={pointerdown ? onpointermove : undefined} />
...
<div class="screen">
	...
	<div class="controls row">
		<div class="col">
			...
			<div class="row">
				<button onclick={() => (n_low = note_to_number("A0"), n_high = note_to_number("C8"))}>88</button>
				<button onclick={() => (n_low = note_to_number("E1"), n_high = note_to_number("G7"))}>76</button>
				<button onclick={() => (n_low = note_to_number("C2"), n_high = note_to_number("C7"))}>61</button>
				<button onclick={() => (n_low = note_to_number("C2"), n_high = note_to_number("C6"))}>49</button>
				<button onclick={() => (n_low = note_to_number("F3"), n_high = note_to_number("F6"))}>37</button>
				<button onclick={() => (n_low = note_to_number("C3"), n_high = note_to_number("C5"))}>25</button>
			</div>
		</div>
		...
		<div class="col">
			<div class="row">
				<input type="range" min="0" max={n_high_white - 1} bind:value={n_low} />
				<span>{number_to_note(n_low_white)}</span>
			</div>
			<div class="row">
				<input type="range" min={n_low + 1} max="87" bind:value={n_high} />
